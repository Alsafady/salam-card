...
<br>
    ...
    <div class="mt-4 ">
        <button type="button" class="btn btn-secondary btn-lg" @click="generateCardH()">تــحميــل</button>
    </div>
...
<div class="mt-4 ">
    <button type="button" class="btn btn-secondary btn-lg" @click="generateCardV()">تــحميــل</button>
</div>
...
export default {
  ...
  props:{
   ...
   methods:{
      async generateCardH() {
        const canvas = await html2canvas(document.getElementById("card-area-h"));
       ...
       async generateCardV() {
           const canvas = await html2canvas(document.getElementById("card-area-v"));
           canvas.style.display = "none";
           document.body.appendChild(canvas);
           const image = canvas.toDataURL("image/png").replace("image/png", "image/octet-stream");
           const a = document.createElement("a");
